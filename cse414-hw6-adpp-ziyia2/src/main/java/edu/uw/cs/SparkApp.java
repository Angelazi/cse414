 ...
 */
// Citation: I looked at the spark starter code that is posted on the website. https://sites.google.com/cs.washington.edu/cse414-20wi/home
 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        ...
        // TODO your code here
        JavaRDD<Row> rdd = d.javaRDD();
        JavaRDD<Row> get_seattle = rdd.filter(t -> t.getString(ORIGIN_CITY_NAME).equals("Seattle, WA"));
        JavaRDD<Row> dest_city = (get_seattle.map(t -> RowFactory.create(t.get(DEST_CITY_NAME)))).distinct();
        return dest_city;
//        d.createOrReplaceTempView("flights");
//        Dataset<Row> result = spark.sql("SELECT DISTINCT destcityname FROM flights WHERE origincityname = 'Seattle, WA' ");
//        JavaRDD<Row> result = result.javaRDD();
//        return result;
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        ...
        // TODO your code here
        JavaRDD<Row> rdd = d.javaRDD();
        JavaRDD<Row> non_cancelled = rdd.filter(t -> t.getInt(CANCELLED) != 1);
        JavaPairRDD<Tuple2<String, Integer>, Integer> pair = non_cancelled.mapToPair(t -> new Tuple2<Tuple2<String, Integer>, Integer>
                (new Tuple2<String, Integer>(t.getString(ORIGIN_CITY_NAME), t.getInt(MONTH)), 1));
        JavaPairRDD<Tuple2<String, Integer>, Integer> result = pair.reduceByKey((v1, v2) -> v1 + v2);
        JavaRDD<Row> final_result = result.map(r -> RowFactory.create(r));
        return final_result;
    }
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        ...
        // TODO your code here
        // ave delay, departing flights, each city
//        JavaRDD<Row> rdd = d.javaRDD();
//        JavaRDD<Row> filter_null = rdd.filter(t -> t.get(DEP_DELAY) != null);
//        JavaPairRDD<String, Tuple2<Integer, Integer>> pair = filter_null.mapToPair(r -> new Tuple2<String, Tuple2<Integer, Integer>>(r.getString(DEST_CITY_NAME), new Tuple2<Integer, Integer>(r.getInt(DEP_DELAY), 1)));
//        JavaPairRDD<String, Tuple2<Integer, Integer>> average = pair.reduceByKey((v1, v2) -> new Tuple2<Integer, Integer>(v1._1() + v2._1(), v1._2() + v2._2()));
//        JavaPairRDD<String, Double> result2 = average.mapToPair(t -> new Tuple2<String, Double>(t._1(), (t._2()._1() * 1.0) / t._2()._2()));
//        JavaRDD<Row> final_result2 = result2.map(r -> RowFactory.create(r));
//        return final_result2;
//
        JavaRDD<Row> rdd = d.javaRDD();
        JavaRDD<Row> q3_output = rdd.filter(l -> l.get(DEP_DELAY) != null);
        JavaRDD<Row> r = q3_output.filter(l -> l.getInt(CANCELLED) != 1);
        JavaPairRDD<String, Tuple2<Integer, Integer>> temp1 = r.mapToPair(a -> new Tuple2<>(a.getString(ORIGIN_CITY_NAME), new Tuple2<Integer, Integer>(a.getInt(DEP_DELAY), 1)));
        JavaPairRDD<String, Tuple2<Integer, Integer>> temp2 = temp1.reduceByKey((v1, v2) -> new Tuple2<>(v1._1() + v2._1(), v1._2() + v2._2()));
        JavaPairRDD<String, Double> temp3 = temp2.mapToPair(t -> new Tuple2<>(t._1(), (t._2()._1() * 1.0) / t._2()._2()));
        JavaRDD<Row> result3 = temp3.map(l -> RowFactory.create(l));
        return result3;
    }
}
